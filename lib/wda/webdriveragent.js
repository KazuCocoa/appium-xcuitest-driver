  ...
  async launch (sessionId) {
    ...
    try {
      ...
      if (this.realDevice) {
        ...
        // TODO: catch and re-start again??
        log.debug(`error in webdriveragent.js======: ${err.message}`);
        log.debug('clean....');
        ...
        if (await fs.exists(installdCacheRoot)) {
          log.debug(`installdCacheRoot: ${installdCacheRoot}`);
          tmpRoot = await tempDir.openDir();
        }
        log.debug('cleaning...');
        if (tmpRoot) {
          log.debug(`tmpRoot: ${tmpRoot}`);
          // Probably we should not delete here since
          // /Users/kazuaki/Library/Developer/CoreSimulator/Devices/ECEDA200-9325-4E7A-9AF8-E63A2903E5A9/data/Library/Caches/com.apple.mobile.installd.staging
          // `Failed to create temporary staging dir` happen
          // await fs.rimraf(installdCacheRoot);

          const deamonInTmp = path.resolve(tmpRoot, INSTALL_DAEMON_CACHE);
          log.debug(`path.resolve(tmpRoot, INSTALL_DAEMON_CACHE): ${deamonInTmp}`);
          if (await fs.exists(deamonInTmp)) {
            await fs.mv(path.resolve(tmpRoot, INSTALL_DAEMON_CACHE), installdCacheRoot, {mkdirp: true});
          }
        }
        log.debug('re-this.xcodebuild.start()');
        return await this.xcodebuild.start();
      }
    }
  }

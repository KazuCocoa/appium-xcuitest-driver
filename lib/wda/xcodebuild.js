...
class XcodeBuild {
  ...
  async createSubProcess (buildOnly = false) {
    ...
    xcodebuild.on('output', async (stdout, stderr) => {
      ...
      if (out.includes('Error Domain=MIInstallerErrorDomain Code=35') && (await tempDir.openDir())) {
        // TODO: Could catch here
        log.debug(`Error Domain=MIInstallerErrorDomain Code=35 happened: ${out}`);
        xcodebuild._wda_error_occurred = true;
        xcodebuild._wda_should_re_build = true;
      }
  ...
  async start (buildOnly = false) {
    ...
    return await new B((resolve, reject) => {
      this.xcodebuild.on('exit', async (code, signal) => {
        ...
        if (this.xcodebuild._wda_error_occurred && this.xcodebuild._wda_should_re_build) {
          log.debug('rejected because of Error Domain=MIInstallerErrorDomain Code=35');
          return reject(new Error(`Error Domain=MIInstallerErrorDomain Code=35`));
        }
      ...
      return (async () => {
        try {
          ...
          if (!this.realDevice && err.message.includes('Error Domain=MIInstallerErrorDomain Code=35')) {
            log.debug('Error happens: Error Domain=MIInstallerErrorDomain Code=35');
            log.debug(err.message);
            try {
              const INSTALL_DAEMON_CACHE = 'com.apple.mobile.installd.staging';
              const installdCacheRoot = path.resolve(this.device.getDir(), 'Library', 'Caches', INSTALL_DAEMON_CACHE);

...
import {
  ...
  normalizePlatformVersion, hasDifferentProcessArguments } from './utils';
import {
...
class XCUITestDriver extends BaseDriver {
  ...
  async start () {
    ...
    // Re-launch the app if the app is running in the background to update processArguments.
    // Except for `noRest`, the app is re-launch or re-installed.
    if (this.opts.noReset && this.opts.bundleId) {
      this.reLaunchAppIfNeeded();
    }
  ...
  async reLaunchAppIfNeeded () {
    this.logEvent('reLaunchAppIfNeededAttempted');

    await commands.activateApp(this.opts.bundleId, {
      arguments: this.opts.processArguments.args || [], environment: this.opts.processArguments.env || {}});

    const checkProcessArguments = async () => {
      log.info(`Checking processArguments for ${this.opts.bundleId}`);
      const response = await this.proxyCommand('/wda/activeAppInfo', 'GET');
      const shouldRelaunch = hasDifferentProcessArguments(
        this.opts.processArguments, response.processArguments);

      if (shouldRelaunch) {
        log.info(`processArguments, '${JSON.stringify(response.processArguments)}', of running app '` +
          `'${this.opts.bundleId}' differes from processArguments in capabilities ` +
          `'${JSON.stringify(this.opts.processArguments)}'`);
        log.info(`Terminate and re-launch '${this.opts.bundleId}' wth '${JSON.stringify(this.opts.processArguments)}'`);
        await commands.terminateApp(this.opts.bundleId);
        await commands.activateApp(this.opts.bundleId, {
          arguments: this.opts.processArguments.args || [], environment: this.opts.processArguments.env || {}});
      }
    };

    await checkProcessArguments();
    this.logEvent('appLaunchedWithGivenProcessArguments');
  }

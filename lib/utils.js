...
/**
 * @typedef {Object} ProcessArguments
 *
 * @property {?array} args - args in ProcessArguments
 * @property {?object} env - env in ProcessArguments
 */

/**
 *
 * @param {?ProcessArguments} capsProcessArguments The processArguments in capabilities
 * @param {?ProcessArguments} actualProcessArguments The processArguments in the response of
 *                                                   get activeAppInfo
 */
function hasDifferentProcessArguments (capsProcessArguments, actualProcessArguments) {
  if (!util.hasValue(capsProcessArguments) && !util.hasValue(actualProcessArguments)) {
    return false;
  }

  const actualEnv = actualProcessArguments.env || {};
  const capsEnv = capsProcessArguments.env || {};

  if (_.xor(capsProcessArguments.args || [],
            actualProcessArguments.args || []).length !== 0) {
    return true;
  }

  const capsEnvKeys = _.keys(capsEnv);
  if (_.xor(capsEnvKeys, _.keys(actualEnv)).length !== 0) {
    return true;
  }
  capsEnvKeys.forEach((key) => {
    if (capsEnv[key] !== actualEnv[key]) {
      return true;
    }
  });
  return false;
